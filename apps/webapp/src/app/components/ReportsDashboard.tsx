                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setSelectedTrackers((prev) =>
                        e.target.checked ? [...prev, tracker.key] : prev.filter((t) => t !== tracker.key)
                      );
                    }}
                  />
                  {tracker.label}
        ...
        <div>
          {noTrackersSelected ? (
            ...
            <div>
              ...
              {error && <p style={{ color: 'red' }}>Error loading report: {error}</p>}
              {!isLoading && !error && reportData && (
                <div>
                  ...
                  {selectedTrackers.includes('sleep') && reportData.sleepSummary && (
                    <div>
                      <h4>Sleep Summary</h4>
                      <p>Total Hours: {reportData.sleepSummary.totalHours?.toFixed(1) ?? 'N/A'}</p>
                      <p>Average Duration: {reportData.sleepSummary.avgDuration?.toFixed(1) ?? 'N/A'} hrs</p>
                      {Array.isArray(reportData.sleepSummary.chartData) &&
                        reportData.sleepSummary.chartData.length > 0 && (
                          <SleepChart
                            data={normalizeChartData(reportData.sleepSummary.chartData)}
                  ...
                  {selectedTrackers.includes('nursing') && reportData.nursingSummary && (
                    <div>
                      ...
                      <p>Total Sessions: {reportData.nursingSummary.totalSessions ?? 'N/A'}</p>
                      <p>Average Duration: {reportData.nursingSummary.avgDuration?.toFixed(1) ?? 'N/A'} min</p>
                      {Array.isArray(reportData.nursingSummary.chartData) &&
                        reportData.nursingSummary.chartData.length > 0 && (
                          <NursingChart data={normalizeChartData(reportData.nursingSummary.chartData)} />
                        )}
                    </div>
                  )}
                  {selectedTrackers.includes('bottle') && reportData.bottleSummary && (
                    <div>
                      ...
                      <p>Total Bottles: {reportData.bottleSummary.totalBottles ?? 'N/A'}</p>
                      <p>Average Volume: {reportData.bottleSummary.avgVolume?.toFixed(1) ?? 'N/A'} mL</p>
                      {Array.isArray(reportData.bottleSummary.chartData) &&
                        reportData.bottleSummary.chartData.length > 0 && (
                          <BottleChart
                            data={normalizeChartData(reportData.bottleSummary.chartData)}
                    ...
                    <div>
                      ...
                      {Array.isArray(reportData.diaperSummary.chartData) &&
                        reportData.diaperSummary.chartData.length > 0 && (
                          <DiaperChart data={normalizeChartData(reportData.diaperSummary.chartData)} />
                        )}
                    </div>
                  )}
                  {}
                    ...
                    <div>
                      ...
                      {Array.isArray(reportData.solidsSummary.chartData) &&
                        reportData.solidsSummary.chartData.length > 0 && (
                          <SolidsChart data={normalizeChartData(reportData.solidsSummary.chartData)} />
                        )}
                    </div>
                  )}
                  {}
                    ...
                    <div>
                      ...
                      {Array.isArray(reportData.medicineSummary.chartData) &&
                        reportData.medicineSummary.chartData.length > 0 && (
                          <MedicineChart data={normalizeChartData(reportData.medicineSummary.chartData)} />
                        )}
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      {Array.isArray(reportData.growthSummary.chartData) &&
                        reportData.growthSummary.chartData.length > 0 && (
                          <GrowthChart data={normalizeChartData(reportData.growthSummary.chartData)} />
                        )}
                    </div>
                  )}
